...
- summary: `ACP(Agent Client Protocol) 실현 가능성을 문서·이슈트래커 조사로 확인했다. ACP는 실재하는 프로토콜이라 용어 사용 자체는 정확했으나 핵심 전제는 정정이 필요하다 — Claude Code는 공식 미지원(이슈가 not planned로 닫힘)이고 존재하는 것은 인증 모델이 다른 서드파티 wrapper뿐이며, Codex도 마찬가지고 Antigravity CLI는 미지원을 로컬에서 직접 확인했다. Gate G2는 낮음~불명으로 판정했다. Windows 설치판 결함 이슈가 있으나 소스 빌드 경로의 성공 사례가 있어 완전 불가능은 아니며 핸즈온 스파이크가 필요하다는 것이 결론이다. 후속 조사로 함의가 해소돼 DONE 처리했다. 전체 원문은 아래 요약(원문) 절에 보존했다.`
