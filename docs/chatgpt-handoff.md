...
8. Project Control currently shows `attention` because the Master Plan declares `Approval state: required`: the next bounded work package for the selected `jarvis-console` needs Owner approval. Historical evidence is verified by branch ancestry since task-0126. That is truthful behavior, not a UI bug.
