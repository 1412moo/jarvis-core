...
- 최근 완료: **Buzz 통합 Phase 1**(task-0039, DONE — task-0040/0041/0043/0045 4/6 완료로 조건 충족. 선택적 잔여 항목이던 task-0042/0044도 이후 완료되어 실질 6/6)
...
- **[2026-08-29] Phase 2 Buzz Bridge Slice 1의 다음 증분 P2-2/P2-3 완료.** P2-2(commit `5cf9930`)는 `orchestrator.js`를 실제 `memory/tasks/<taskId>.md`에 연결했다(append-only, taskId별 OS temp `wx` lock으로 동시 실행 방어, 오프라인 테스트 24/24 PASS). P2-3(commit `11e74e6`)은 append 포맷이 Jarvis task lifecycle parser(`task_file_writer.py`)와 충돌해 `transition_task_file_status()`/`record_task_completion_evidence()`를 hold시키던 문제를 bullet 마커 변경(`-`→`*`)만으로 수정하고 task lifecycle/승인 경계 회귀 테스트 7건을 추가했다(전체 31/31 PASS). 이후 회귀 재검토에서도 **G1 PARTIAL / G2 PASS / G3 FAIL(비차단)** 판정은 그대로 유지됨을 확인했다. 상세 provenance: `memory/tasks/task-0049-buzz-bridge-p2-2-p2-3-completion.md`.
- **[2026-09-03] P2-4/P2-5 구현 완료(commit `b270f63`) + P2-6 Relay 실통합 검증 PASS.** P2-4는 `adapters/discord/bot_minimal.py`의 `on_message`에 단일 인가 게이트를 추가해 `/approve`·`/run`·`/retry`·`/task`를 Owner 전용으로 만들었다(allowlist는 환경변수 `JARVIS_OWNER_DISCORD_USER_IDS`, 미설정 시 **기동 거부**, 자연어 경로가 합성한 `/approve`도 동일 게이트 통과, read-only 명령 6종은 개방 유지, `_run_command` 시그니처와 승인 파이프라인 무수정). P2-5는 확정된 승인 결과만 전용 채널 `jarvis-task-status-notifications`에 **아웃바운드 단방향**으로 발행한다(신규 `orchestrator/buzz-bridge/publish_notification.js`, 승인 결과 무변조·publish 실패는 별도 보고, `p`/`jarvis-run` 태그 부재로 bridge 실행 게이트 재진입 불가, `EXECUTION_SCRIPT_WHITELIST` 미재사용). P2-6은 코드 변경 없이 실제 relay로 `/approve` → authoritative 전이 → publisher → Nostr publish → **독립 observer 수신**까지 전 구간을 검증해 전건 PASS했다(kind 9, `p`/`jarvis-run` 없음, live event를 실제 `passesInboundGate`에 투입해 거부 확인). 이로써 **§6 ③(승인 게시 흐름)의 아웃바운드 절반이 완료**됐고, **Buzz → Jarvis 인바운드 승인 입력은 Owner 결정으로 범위 밖**이다. 상세 provenance: `memory/tasks/task-0050-buzz-bridge-p2-4-p2-5-p2-6-completion.md`.
- **[2026-09-06] 승인 경로 감사 기록화와 task 기록 계약 정정 완료(task-0051~0057).** `task-0051`(`c199e7f`)은 이 문서의 구조화 필드가 Console 파서 계약을 깨뜨린 것을 정정했다. `task-0052`(설계 `59bfdbd`, Owner 결정 7건 `3c5ee1b`, 구현 `a6c4ef3`)는 `task-0044`의 해시체인을 `/approve`·`/run`·`/retry`에 실연동했고, **감사 append가 실패하면 승인을 거부**한다(fail-closed). 그 과정에서 드러난 canonical parser 경계 결함은 `task-0054`(`38b9027`)가 hotfix로 분리해 고치고 상태 전이표를 2쌍에서 6쌍으로 넓혔다. `task-0053`(`41501a5`, 기록 `3f76d6d`)은 실행 결과 전이를 durable(fsync+atomic replace+digest)로 바꾸고 task 파일 저장 필드를 13개에서 **8개**로 줄였다(나머지 5개는 판독 시 파생). `task-0055`(`8e6278a`)는 메타데이터와 전이를 **단일 원자 쓰기**로 묶어 `execution_status=success`인데 `status=DOING`인 자기모순 창을 닫았고, `6033650`은 코드가 수행하면서도 표에 없던 `DOING → FAILED` 전이를 정정했다. `task-0056`(`59e7d65`)이 남은 canonical FAIL 8건을 정리해 **전수 PASS**를 복구하고 테스트 예외 목록을 제거했으며, `task-0057`(`c2efa99`)이 `docs/task-model.md` §10을 강제 스키마에 맞춰 정정했다. 이 시점의 체인은 기록만 하고 **운영 경로에서 검증되지는 않았다** — 그 공백은 아래 `task-0066`이 메웠다.
...
- 현재 다음 작업: 선택된 `jarvis-console`(selected_for_proposal)에서 축소된 Console을 실제 task lifecycle로 반복 사용하고 다음 bounded work package 1개를 제안한다. task-0038 §6 Phase 2 통합 순서의 남은 단계 **②Discord intake 연결 / ④Reviewer·QA agent 등록+worktree 격리 / ⑤Discord 3개월 병행**은 유의미한 scope 확장이며 **미승인 선택적 확장으로 보류한다 — 별도 Owner의 명시적 결정 없이는 착수하지 않는다.** (③승인 게시 흐름은 아웃바운드가 P2-5/P2-6으로 완료됐고 인바운드는 범위 밖이므로 이 목록에서 제외한다.) 승인 전까지 진행 가능했던 Phase 1 잔여 항목 `task-0042`(역할별 서명키, commit `1557ad7`)/`task-0044`(감사 해시체인, commit `116fe2d`)는 **둘 다 `DONE`**이다. 이후 `task-0052`가 그 해시체인을 `/approve`·`/run`·`/retry` 경로에 실제로 연결했다(commit `a6c4ef3`).
...
- **[2026-09-11] rolling-window 구조에 대한 Owner 결정 2건(task-0124).** ① 완료된 milestone 의 evidence 가 5커밋 뒤 `git log -n 5` 창 밖으로 밀렸다는 이유만으로 Manager Report 가 영구 blocked 되는 것은 의도한 장기 운영 모델이 아니다 — historical evidence 의 시간적 유효성 상실과 current operational blockage 를 동일시하지 않는다. ② 아래 §4 package 표는 "현재 milestone 의 작업 목록"이 아니라 **완료·검증된 milestone 의 historical evidence** 로 정의하며, 기존 commit hash 는 historical fact 로 **보존하고 최신 HEAD 로 갱신하지 않는다.** 표시용 recent-5 계약과 검증용 evidence 를 분리하는 방향(D1)만 설계 대상으로 승인했고 window 숫자·TTL·schema·validator 는 정하지 않았다. **production 구현은 후속 task 다** — 그때까지 카드가 `attention` 인 것은 정상 귀결이다
- **[2026-09-11] D1 구현 완료(task-0126).** task-0124/0125 가 승인·설계한 historical evidence 분리를 구현했다. 조사에서 드러난 사실은 기존 검증이 commit 의 **존재를 확인한 적이 없다**는 것이었다 — allowlist 에 임의 commit 을 물을 수 있는 명령이 없어 `absent from Git evidence` 가 실제로는 "최근 5 개 안에 없다"를 뜻했고, 실재하는 오래된 commit 과 조작된 hash 가 구분되지 않았다. 이제 `merge-base --is-ancestor` 로 **이 branch 역사의 일부인가**를 묻는다. history 는 자라기만 하므로 답이 만료되지 않아 **window 숫자도 TTL 도 필요 없고 schema 변경도 0** 이다. 표시용 recent-5 계약(`MAX_COMMITS = 5`)과 §4 historical hash 는 **그대로 유지**된다. 결과: Manager/Director 가 `blocked` → `milestone_complete` 로 바뀌고 git evidence conflict 3 건이 사라졌다. 카드가 계속 `attention` 인 것은 §2 의 `Approval state: required` 때문이며 이는 실제로 참인 Owner 결정 대기다
- **[2026-09-17] Console 실사용 사이클 2회 완결(task-0124/0125 종료, task-0130, task-0131).** 설계 기록만 남고 `TODO` 였던 task-0124/0125 를 Owner 가 Console 에서 Start → Evidence → Complete 로 닫았다(`4fea24d`). 그 사용에서 Complete Preview 가 증거 기록을 요구하면서 기록 값을 보여주지 않는 점이 드러나 task-0130(`4ce60c4`, 종료 `991511f`)이 기록 값 또는 `Not recorded` 를 표시하게 했다 — 표시는 평가가 아니며 Complete 를 승인하거나 차단하지 않고 D1 원칙과 경고 문구는 그대로다. task-0130 dogfooding 중 빈 입력의 Record Evidence 가 서버 오류 코드를 Complete Preview 아래에 띄워 오해를 낳았고, task-0131(`5b9a774`)이 빈/공백 입력은 요청 없이 안내하고 보유한 Evidence Preview 를 비우게 했다. 서버 1–500 자 검증은 무변경이다. task-0131 은 repair 3 회(Owner 가 budget 을 3 으로 증액) 후 Reviewer/QA PASS 였고, 그 결과는 검증된 candidate 위의 task 기록 전용 commit(`32a3d22`)으로 남겼다 — Owner 는 이 commit 을 새 candidate 가 아닌 결과 기록으로 취급하도록 이 task 에 한해 결정했으며 SOP 규칙은 바뀌지 않았다. 두 task 모두 Owner 가 Console 에서 Complete 했다(`991511f`, `5b3f617`). `Approval state: required` 와 task-0038 §6 ②/④/⑤ 보류는 그대로다
- **[2026-09-17~21] Reviewer 승인 원문 결속·정의 통일·호출 Skill(task-0133~0136).** task-0132에서 Owner가 승인 메시지로 제외한 변경이 Reviewer PASS·QA PASS를 통과한 일을 계기로 SOP에 Manager의 승인 조건 원문 인용과 충돌 시 구현 전 질문, Reviewer의 원문 대조를 추가했다(task-0133). Reviewer 정의는 승인 원문이 없으면 BLOCKED로 멈추고(task-0134), Claude·Codex 두 Reviewer 정의는 같은 역할 규칙을 갖고 validator가 양쪽을 검사한다(task-0135). Reviewer 호출문 조립은 명시 호출 전용 Claude Skill prototype `jarvis-reviewer-call`로 고정했다(task-0136).
- **[2026-09-22~24] Studyroom과 Reviewer 누적 검토 명령(task-0137~0140).** `apps/studyroom/`에 Jarvis-Core 자체를 교재로 삼는 로컬 read-only 학습 앱을 추가했다(task-0137). 더블클릭 실행기와 Recordroom 26건·Glossary 47개·Learn 18개·Technologies 11개·Features 11개 콘텐츠 확장은 task 기록 없이 진행돼 task-0138로 사후 기록했다. Reviewer 정의에 baseline 누적 diff·commit 범위·candidate 시점 `git show`·evidence path를 추가하고 작업 트리 직접 읽기 금지를 명시했다(task-0139). task-0137 summary 메타데이터와 Learn 연결 1건을 정정했다(task-0140).
- 현재 결정 필요: **선택된 `jarvis-console`의 다음 bounded work package 제안 검토·승인(구현 착수 전)**. task-0038 §6 Phase 2 통합 순서의 남은 단계(②Discord intake 연결/④Reviewer·QA agent 등록+worktree 격리/⑤3개월 병행)는 **보류된 선택적 확장(현재 결정 대상 아님, 재개 시 별도 Owner 결정)**이다. 기존에 기록된 **Codex/agy bridge 확장 여부, bridge/relay supervisor 도입 여부, 승인 감사 기록에 승인자 ID 추가 여부**는 그대로 유지한다 — 그 외 Phase 1 관련 결정과 P2-2/P2-3, P2-4/P2-5/P2-6은 모두 완료(task-0038, task-0050 참고). ③승인 게시 흐름은 아웃바운드 완료·인바운드 범위 밖으로 확정되어 결정 대상에서 빠졌다. 단 **승인자 ID 추가는 단순 선택지가 아니다** — 채택하려면 ①P2-4의 "승인 파이프라인은 신원을 모른다" 불변식(`on_message`의 게이트만 `author_id`를 보고 `_run_command`는 받지 않는다), ②감사 스키마 3곳(`KIND_ACTOR_MAP`의 역할 상수 `owner`/`orchestrator`, `FORBIDDEN_PAYLOAD_KEYS`의 `user_id`/`author_id`/`discord_user_id`/`owner_id`, payload 키 집합 완전 일치 검사), ③회귀 테스트 `audit_payload_has_no_owner_identity`를 **함께** 바꿔야 한다(task-0044/0052)
...
- Manager reporting next package ID: `buzz-bridge-phase2-slice1-increment-v0.1`
- Current workstream: jarvis-console — primary workstream(task-0121, Owner decision selected_for_proposal). 축소 완료 후 문서 정합화(task-0122/0123), historical evidence ancestry 검증(task-0126), Owner 선택 표시(task-0127/0128), 실사용 사이클 2회(task-0124/0125 Console 종료, Complete Preview evidence 표시 task-0130, 빈 evidence 입력 안내 task-0131). Buzz Bridge Phase 2 Slice 1~P2-6은 완료된 이력이고 task-0038 §6 Phase 2 통합 순서의 남은 단계(②/④/⑤)는 미승인 선택적 확장으로 보류
- Current milestone: Jarvis Console 축소 완료(task-0071/0073/0074/0075/0076) + primary workstream 확정(task-0121) + Owner 선택 표시(task-0127/0128) + Console 실사용 사이클 2회 완결(task-0130/0131) + Reviewer 규칙 강화(task-0133~0135/0139)와 호출 Skill(task-0136) + Studyroom 학습 앱과 콘텐츠 확장(task-0137/0138/0140). Buzz 통합 Phase 1(task-0039)과 Phase 2 Buzz Bridge Slice 1~P2-6(task-0048~0050)은 완료 이력이며 task-0038 §6 Phase 2 통합 순서의 남은 단계(②/④/⑤)는 미승인 보류 — Director Dashboard v0.1B는 계속 보류
- Recommended next step: Owner가 선택한 jarvis-console(selected_for_proposal)에서 축소된 Console을 실제 task lifecycle로 반복 사용하고, 그 실사용 관찰을 근거로 다음 bounded work package 1개를 제안받아 Owner가 승인 여부를 결정한다. task-0038 §6 Phase 2 통합 순서의 남은 단계(②Discord intake 연결/④Reviewer·QA agent 등록+worktree 격리/⑤3개월 병행)는 미승인 선택적 확장으로 보류되며 이 단계의 선행조건이 아니다. task-0042가 ④의 선행조건인 것은 사실이지만 그 사실이 ④ 착수를 승인하지는 않는다
- Next user-visible milestone: 축소된 Jarvis Console을 실제 Jarvis task lifecycle 화면으로 반복 사용 (task-0038 승인 항목 ②, D1 결정 — 승인 화면이 아니며 NEEDS_APPROVAL 전이는 추가하지 않는다)
...
- Recent completed: Manual Pilot v0.1A 사이클 완료. Buzz 통합 Phase 1(task-0039), 핸즈온 스파이크 S1~S7(task-0047), Buzz Bridge Slice 1~P2-6(task-0048~0050, `a54316f`~`b270f63`) 완료. 감사 해시체인 승인 경로 연동과 durable·단일 원자 쓰기(task-0052/0053/0055), task 기록 canonical 검증 전수 PASS(task-0056/0057). Reviewer 승인 원문 결속·두 정의 통일·누적 검토 명령(task-0133~0135/0139)과 호출 Skill(task-0136). Studyroom 학습 앱·더블클릭 실행기·콘텐츠 확장(task-0137/0138/0140).
- Approval state: required
- Approval note: Owner는 jarvis-console을 선택했다(selected_for_proposal). 다음 bounded work package는 제안(work_package_proposal_only)까지만 허용되며 구현 착수에는 그 package에 대한 Owner 승인이 필요하다. SOP v0.1B 승격은 승인됐고 Dashboard, 자동 runtime, push, PR, 외부 호출과 jarvis.bat는 범위 밖이다. task-0038 §6 Phase 2 통합 순서의 남은 단계(②/④/⑤)는 미승인 선택적 확장으로 보류 중이며 현재 결정 대상이 아니다 — 착수 전 별도 승인이 필요하다. Buzz → Jarvis 인바운드 승인 입력은 Owner 결정으로 범위 밖이고, director-dashboard-v0.1b는 미승인 상태로 보류다.
...
**[2026-09-17~24] SOP 보강.** Manager는 Owner 승인 조건을 원문 그대로 인용해 Reviewer·QA에 넘기고,
조건끼리 충돌하면 구현 전에 묻는다(task-0133). Reviewer 정의는 `=== OWNER APPROVAL (verbatim) ===` 아래
승인 원문이 없으면 BLOCKED로 멈춘다(task-0134). `.claude/agents/reviewer.md`와 `.codex/agents/reviewer.toml`은
candidate hash fail-closed를 포함한 같은 역할 규칙을 갖고 validator가 두 파일을 모두 검사한다(task-0135).
Manager가 baseline 전체 hash와 evidence path를 주면 Reviewer는 baseline..candidate 누적 diff·commit 범위와
candidate 시점 파일을 hash에 묶인 git 형식으로만 읽는다(task-0139).
